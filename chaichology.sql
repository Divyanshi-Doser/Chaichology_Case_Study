...
INSERT INTO Tea_Shops (shop_id, shop_name, city, state, country)
VALUES (1, 'Chaichology', 'Mumbai', 'Maharashtra', 'India'), 
...
-- Menu Items Table 
 --(Definiing enum type first)

CREATE TYPE categeory_enum AS ENUM ('Tea', 'Snack', 'Dessert');  

CREATE TABLE Menu_Items ( 
    item_id SERIAL PRIMARY KEY, 
    item_name VARCHAR(100) NOT NULL, 
    category categeory_enum NOT NULL, 
    price DECIMAL(5,2) NOT NULL, 
    available BOOLEAN DEFAULT TRUE 
); 
 
INSERT INTO Menu_Items (item_id, item_name, category, price, available)
VALUES (1, 'Masala Chai', 'Tea', 30.00, TRUE), 
...
INSERT INTO Sales (shop_id, item_id, quantity, transaction_date)
VALUES (1, 1, 10, '2025-03-01'), 
       (1, 2, 5, '2025-03-02'), 
       (2, 3, 12, '2025-03-02'), 
       (3, 4, 6, '2025-03-03'), 
       (4, 5, 8, '2025-03-04'), 
...
SELECT m.item_name, COUNT(s.sale_id) AS total_orders
FROM Sales AS s
...
SELECT t.shop_name, t.city, SUM(m.price * s.quantity) AS revenue
FROM Sales  AS s
JOIN Tea_Shops AS t
...
SELECT m.item_name, SUM(s.quantity) AS total_quantity
...
SELECT t.shop_name, t.city, COUNT(r.rating) AS total_ratings
FROM Tea_Shops AS t
JOIN Ratings AS r 
ON t.shop_id = r.shop_id
GROUP BY t.shop_name, t.city
ORDER BY total_ratings DESC;

-- 7) List All Shops with an Average Rating Above 4.5 ? 
SELECT t.shop_name, t.city, ROUND(AVG(r.rating),1) AS avg_rating
FROM Tea_Shops AS t
JOIN Ratings AS r 
ON t.shop_id = r.shop_id
GROUP BY t.shop_name, t.city
...
JOIN Ratings AS r 
ON t.shop_id = r.shop_id
WHERE t.city LIKE 'Bangalore' AND r.review LIKE '%amazing%';
...
JOIN Menu_items AS m
ON s.item_id = m.item_id
WHERE m.item_name LIKE 'Masala Chai'
GROUP BY r.customer_name, t.shop_name, t.city;

-- 10) Find the day with the most sales ? 
SELECT transaction_date AS date, SUM(quantity) AS quantity_sold
FROM Sales
GROUP BY date
